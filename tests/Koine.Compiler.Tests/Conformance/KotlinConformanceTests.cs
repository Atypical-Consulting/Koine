...
public class KotlinConformanceTests
{
    ...
    /// <summary>
    /// Issue #1866 — the #1511 fix (Rust) never ported to Kotlin: a command's <c>result</c> expression
    /// never numerically reconciled against the command's declared return type, unlike the identical
    /// decision already applied at factory ctor args (#1732) and coalesce operands (#1615). An
    /// <c>Int</c>-typed member returned against a <c>: Decimal</c> return type emitted a bare <c>Long</c>
    /// return where a <c>BigDecimal</c> is required — a real <c>kotlinc</c> "type mismatch" error.
    /// </summary>
    [Fact]
    ...
    /// <c>total: Decimal = amount + surcharge</c> (references siblings → a get-only property) from
    /// <c>total: Decimal = 5</c> (a constant → a stored default); #1880 reconciled only the stored half,
    /// leaving the derived property's BODY unreconciled against its own declared type. Kotlin emitted
    /// <c>val total: java.math.BigDecimal get() = this.amount + this.surcharge</c> — a <c>Long</c> body
    /// under a <c>BigDecimal</c> declaration, a hard <c>kotlinc</c> type mismatch. Routed through the very
    ...
    public void Derived_member_body_widens_an_int_to_bigdecimal_against_its_declared_type()
    {
        ...
        var money = result.Files.Single(f => f.RelativePath.EndsWith("Money.kt", StringComparison.Ordinal)).Contents;
        money.ShouldContain("val total: java.math.BigDecimal get() = java.math.BigDecimal.valueOf(this.amount + this.surcharge)");
        money.ShouldNotContain("val total: java.math.BigDecimal get() = this.amount + this.surcharge");

        var invoice = result.Files.Single(f => f.RelativePath.EndsWith("Invoice.kt", StringComparison.Ordinal)).Contents;
        invoice.ShouldContain("val total: java.math.BigDecimal get() = java.math.BigDecimal.valueOf(this.amount + this.surcharge)");
        invoice.ShouldNotContain("val total: java.math.BigDecimal get() = this.amount + this.surcharge");
    ...
    /// <summary>
    /// Issue #1888 — the optional-declared variant (Rust's #1329 shape). Kotlin nullability is subtyping,
    /// so only the NUMERIC dimension renders: the body still widens to <c>BigDecimal</c>, and a
    /// non-optional value flows into a nullable-declared property unchanged (no <c>NeedsSomeWrap</c> lift,
    /// which is why the <c>String? = tag</c> member stays byte-identical here while Java's lifts).
    /// </summary>
    [Fact]
    public void Optional_declared_derived_member_body_widens_without_a_lift()
    {
        ...
        var money = result.Files.Single(f => f.RelativePath.EndsWith("Money.kt", StringComparison.Ordinal)).Contents;
        money.ShouldContain("val total: java.math.BigDecimal? get() = java.math.BigDecimal.valueOf(this.amount + this.surcharge)");
        money.ShouldContain("val label: String? get() = this.tag");
    ...
    public void Matching_type_and_non_numeric_derived_member_bodies_render_unchanged()
    {
        ...
        var money = result.Files.Single(f => f.RelativePath.EndsWith("Money.kt", StringComparison.Ordinal)).Contents;
        money.ShouldContain("val exact: java.math.BigDecimal get() = this.price");
        money.ShouldContain("val label: String get() = this.tag");
        money.ShouldContain("val count: Long get() = this.qty");
    ...
    public void Emit_payload_argument_widens_an_int_member_to_bigdecimal_against_a_decimal_declared_field()
    {
        ...
        var invoice = result.Files.Single(f => f.RelativePath.EndsWith("Invoice.kt", StringComparison.Ordinal)).Contents;
        invoice.ShouldContain("Charged(java.math.BigDecimal.valueOf(this.tax))");
        invoice.ShouldNotContain("Charged(this.tax)");
    ...
    /// widening as <c>emit</c> above — a published integration event's <c>Decimal</c>-declared field fed
    /// an <c>Int</c>-typed argument emitted an uncoerced value too.
    /// </summary>
    [Fact]
    public void Publish_payload_argument_widens_an_int_member_to_bigdecimal_against_a_decimal_declared_field()
    {
        ...
        var invoice = result.Files.Single(f => f.RelativePath.EndsWith("Invoice.kt", StringComparison.Ordinal)).Contents;
        invoice.ShouldContain("ChargedOut(java.math.BigDecimal.valueOf(this.tax))");
        invoice.ShouldNotContain("ChargedOut(this.tax)");
    ...
    /// <summary>
    /// Issue #1866, the <c>NeedsOptionalWiden</c> composition: an already-nullable <c>Int?</c> member
    /// <c>result</c>-ed into a nullable-declared <c>Decimal?</c> return must widen via
    /// <c>?.let { BigDecimal.valueOf(it) }</c>, never a bare <c>BigDecimal.valueOf(...)</c> wrap around a
    /// <c>Long?</c> — the #1335/#1343 bug class the ctor-arg reconciler already guards.
    /// </summary>
    [Fact]
    public void Result_expression_widens_an_optional_int_member_via_let_against_an_optional_decimal_return_type()
    {
        const string src =
            "context Billing {\n" +
            "  entity Invoice identified by InvoiceId {\n" +
        ...
        var invoice = result.Files.Single(f => f.RelativePath.EndsWith("Invoice.kt", StringComparison.Ordinal)).Contents;
        invoice.ShouldContain("return this.maybeTax?.let { java.math.BigDecimal.valueOf(it) }");
    ...
    /// result-expression reconciliation — a real <c>kotlinc</c> "type mismatch" error.
    /// </summary>
    [Fact]
    ...
    public void Result_and_payload_of_matching_or_non_numeric_types_are_unaffected()
    {
        ...
        invoice.ShouldContain("Noted(this.memo)");
