...
/// independent <see cref="PhpEmitter"/> instances must produce character-for-character identical
...
/// </summary>
public class PhpDeterminismTests
{
    /// <summary>
    /// Compiles <see cref="PhpSnapshotTests.Fixture"/> twice with two fresh emitter instances
    ...
    public void Emitting_twice_is_byte_identical()
    {
        ...
        var result1 = compiler.Compile(PhpSnapshotTests.Fixture, new PhpEmitter());
        result1.Success.ShouldBeTrue("First compile failed: " + string.Join("\n", result1.Diagnostics.Select(d => d.ToString())));

        var result2 = compiler.Compile(PhpSnapshotTests.Fixture, new PhpEmitter());
