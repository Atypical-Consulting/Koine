...
public class RustEmitterTests
{
    ...
    /// compare against the <c>Decimal</c>-typed <c>rate</c>. The closure parameter itself renders
    /// <c>|&amp;n|</c> (#1801): <c>Int</c> is <c>Copy</c>, so the binder deref-binds by value the same way
    /// every other by-value-binding target does.
    /// </summary>
    [Fact]
