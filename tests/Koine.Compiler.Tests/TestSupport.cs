...
public static class TestSupport
{
    ...
    /// </summary>
    /// <remarks>
    /// The machinery itself now lives in <see cref="GeneratedAssemblyCompiler"/> (the product-side
    /// <c>Koine.Execution</c> assembly, issue #236) so the scenario runner and the meta-tests share one
    /// harness; this stays as the suite's spelling of it — same signature, same default load context
    /// (<see cref="AssemblyLoadContext.Default"/>), so every existing meta-test is unaffected.
    /// </remarks>
    public static (Assembly? Assembly, IReadOnlyList<string> Errors) Compile(
        ...
        GeneratedAssemblyCompiler.Compile(
            files as IReadOnlyList<EmittedFile> ?? files.ToList(), runRegexGenerator);
