...
| `publish X(field: expr, …)` in an aggregate-root `command` | records the integration event `X` into the root's `IntegrationEvents` collection — the cross-context counterpart of `emit`, which records an intra-aggregate **domain** event. With `--layers application` the handler relays it to `IUnitOfWork.Enqueue` *before* the commit, so the `--layers infrastructure` outbox rows land in the same transaction. The context must still declare `publishes X` |
...
| `@route("/orders/{id}")`, `@get`/`@post`/`@put`/`@delete`/`@patch`, `@auth("admin")` before a `command`/`query`/`create` factory | nothing in the domain C#; the HTTP consumers read them — the `api` layer (`--layers api`) maps `endpoints.MapPut("/orders/{id}", …).RequireAuthorization("admin")`, and `--target openapi` keys the operation at that path/verb with a `security` requirement. Each axis falls back to the convention (command → `POST /{entity}/{command}`, factory → `POST /{entity}/{factory}`, query → `GET /{query}`) on its own, and both consumers derive it from one shared source, so they never disagree. A factory's `{id}` token has no aggregate-identity fallback the way a command's does — a factory mints the identity it creates, so its request record has no identity property to bind; on a Guid identity it cannot even take an `id` parameter (`KOI0807` reserves the name), so `{id}` there is always unbound — name the token after a real parameter, or use the explicit-id opt-in a `natural`/`sequence` identity allows |
