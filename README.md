...
| `@route("/orders/{id}")`, `@get`/`@post`/`@put`/`@delete`/`@patch`, `@auth("admin")` before a `command`/`query` | nothing in the domain C#; the HTTP consumers read them — the `api` layer (`--layers api`) maps `endpoints.MapPut("/orders/{id}", …).RequireAuthorization("admin")`, and `--target openapi` keys the operation at that path/verb with a `security` requirement. Each axis falls back to the convention (command → `POST /{entity}/{command}`, query → `GET /{query}`) on its own. A `create` factory carries none of these annotations, but it is not invisible to the same two consumers: it always maps `POST /{entity}/{factory}` in the `api` layer and gets its own `openapi` operation (a `200` for the created aggregate), and it claims that route just like a command/query would if two declarations collide on it |
