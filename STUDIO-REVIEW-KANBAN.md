...
| F5 | **HIGH** | Code → Compatibility tab | `view-check` panel renders **completely empty** (0 children, no text, no empty state). Tab is reachable but shows a black void. Must wire it up or show a meaningful empty/explanatory state. | ✅ fixed — `inspectorController` `renderCheckIdleIfEmpty()` now paints a "Model compatibility" idle state with a "Check against baseline…" trigger (or a Chromium-only note); see the Done table. |
| F6 | medium | Right rail → Rules tab | `rview-rules` showed literal **"Coming soon."** — unfinished feature exposed in UI. | ✅ resolved — Rules retired in **#730**: a selected element's invariants surface in the Properties panel's Invariants section. No `rview-rules` / placeholder remains; regression-guarded in `RightStrip.test.tsx` (#759). |
| F7 | medium | Right rail → Notes tab | `rview-notes` showed literal **"Coming soon."** — unfinished feature exposed in UI. (Distinct from the center Documentation→Notes, which IS implemented.) | ✅ resolved — Notes retired in **#730**: model Notes live in the center Deck's Docs surface. No `rview-notes` / placeholder remains; regression-guarded in `RightStrip.test.tsx` (#759). Future incomplete surfaces hide via `panelGate` (#759), never a bare placeholder. |
...
| #759 | **Preact migration finished (the #193 foundation).** The interactive chrome hosts now render via Preact: a capability **gate** (`panelGate.ts`) + a migration recipe (`CONTRIBUTING-preact-migration.md`); the **right strip** (`RightStrip.tsx`), **left rail** (`LeftRail.tsx`), and **AI-Chat host** (`AssistantView.tsx`, a thin host around the imperative `aiPanel`) replace their `*Markup()` string builders; the **export menu** is consolidated into `ExportMenu.tsx`; `SourceControlPanel` gained a story. Each renders once into its `index.html` thin-shell host, so the controller's captured nodes + the imperative islands in `#filetree-body`/`#rail-domain-pane` are never reconciled away. CodeMirror, the file explorer, maxGraph, `aiPanel`, and the `src/host` seam stay intentionally imperative (non-goals). Verified: tsc + `npm run build` + 2218 vitest green. | ✅ fixed |
