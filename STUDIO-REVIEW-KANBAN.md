...
| F6 | medium | Right rail → Rules tab | `rview-rules` showed literal **"Coming soon."** — unfinished feature exposed in UI. | ✅ resolved — Rules retired in **#730**: a selected element's invariants surface in the Properties panel's Invariants section. No `rview-rules` / placeholder remains; regression-guarded in `rightStrip.test.ts` (#759). |
| F7 | medium | Right rail → Notes tab | `rview-notes` showed literal **"Coming soon."** — unfinished feature exposed in UI. (Distinct from the center Documentation→Notes, which IS implemented.) | ✅ resolved — Notes retired in **#730**: model Notes live in the center Deck's Docs surface. No `rview-notes` / placeholder remains; regression-guarded in `rightStrip.test.ts` (#759). Future incomplete surfaces hide via `panelGate` (#759), never a bare placeholder. |
