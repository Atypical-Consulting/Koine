...
- **`@route("…")`/`@get`…`@patch`/`@auth("role")` on commands/queries/factories (W2)** ✅ **Done (#1219; extended to `create` factories in #1846)** — one grammar change (`annotation*` on `commandDecl` `:162` / `queryDecl` `:126`, then `factoryDecl` `:194`), read in `KoineModelBuilderVisitor.ReadApiAnnotations`, validated in `CqrsValidator.ValidateApiAnnotations` (`KOI1208`–`KOI1210`, `KOI1212`–`KOI1215`) plus the context-level `CqrsValidator.ValidateApiRoutes` (`KOI1211`, route+verb collisions), and consumed by both HTTP targets off the shared `RouteInfo`. A factory diverges on exactly one rule: its `{id}` route token gets **no** aggregate-identity fallback (it mints its identity, so the request record has no identity property), which is why `RouteDerivation.ForFactory` and the factory's `ValidateApiAnnotations` call both pass a `null` identity. `usecase` was left alone: it has no HTTP surface to override.
