...
## Polyglot demos (TypeScript, Python, PHP, Rust)

Koine's C# emitter is the most complete target and gets the full six-context pizzeria demo above.
Each of the other four shipped code emitters gets its own smaller, runnable, self-checking demo
(issue #1073) — the polyglot analogue of this one. They all regenerate the **same** single-context
model, [`templates/starters/ordering`](../templates/starters/ordering) (an `Order` aggregate with
an `OrderLine` value object and an `OrderStatus` state machine), to that target with the Koine CLI,
then — toolchain permitting — build/type-check and run a small hand-written driver that constructs
the generated `Order`, drives it, and **asserts real values** (never emitted formatting/whitespace,
so these demos never churn when the emitter's output shape changes, only when its behavior does):

| Demo | Target | Type-check | Reference snapshot |
|------|--------|------------|---------------------|
| [`demo/typescript`](../demo/typescript) | TypeScript | `tsc --strict` | [`reference/`](../demo/typescript/reference) — `Order.ts.txt`, `OrderLine.ts.txt` |
| [`demo/python`](../demo/python) | Python | `mypy --strict` | [`reference/`](../demo/python/reference) — `order.py.txt`, `order_line.py.txt` |
| [`demo/php`](../demo/php) | PHP | `php -l` + `phpstan analyse --level max` | [`reference/`](../demo/php/reference) — `Order.php.txt`, `OrderLine.php.txt` |
| [`demo/rust`](../demo/rust) | Rust | `cargo run` (the compile step itself) | [`reference/`](../demo/rust/reference) — `ordering.rs.txt` |

Each `reference/` folder mirrors this one (`demo/reference/`, see [its own README](../demo/reference/README.md)
for the pizzeria/C# snapshots): a couple of representative emitted source files committed for
browsing without running the tool. The live `generated/` tree under each `demo/<lang>/` is
git-ignored and regenerated by every run.

Run any single demo directly:
...
...or run the C# demo plus all four polyglot demos in one pass with the orchestrator, which prints
a per-demo PASS/SKIP/FAIL summary and exits non-zero only if a demo produced a real FAIL:

```bash
bash demo/build-all.sh
```

**Toolchain gating.** Every `demo/<lang>/run.sh` always regenerates and attempts its language's
type-check first; only then does it gate on that language's toolchain (`tsc`+`node`, `mypy`+
`python`, `phpstan`+`php`, or `cargo` — honoring the same `KOINE_TSC`/`KOINE_NODE`/`KOINE_MYPY`/
`KOINE_PYTHON`/`KOINE_PHPSTAN`/`KOINE_PHP`/`KOINE_CARGO` overrides the `Conformance/` test suites
do). Locally, a missing toolchain makes that demo **exit 3** and print an install notice — a clean
skip, not a failure. In CI, every toolchain is installed and `KOINE_REQUIRE_CONFORMANCE` is set, so
the same gate becomes a **hard failure** instead of a silent pass if a toolchain ever drops off a
runner. The shared xUnit harness at
[`tests/Koine.Compiler.Tests/DemoBuildTests.cs`](../tests/Koine.Compiler.Tests/DemoBuildTests.cs)
shells each `run.sh` and funnels that same exit-code contract through
`TestSupport.RequireOrSkip` — exactly like the `Conformance/` suites — so `dotnet test` shows a
`Skipped` (not `Failed`) result locally when a language's toolchain isn't installed, and CI proves
all four for real on every push.
