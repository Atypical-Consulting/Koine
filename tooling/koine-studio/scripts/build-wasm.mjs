...
//    when KOINE_WASM_AOT is truthy — the deployed studio-web build (koine-studio.yml) sets it so
//    users get the faster compiler, while a local `npm run dev:web`/`build:web` (run via
...
// 2. Replace public/koine-wasm/ with the fresh bundle (_framework + main.js).
