...
/**
 * Re-map a worker-call rejection: the worker's bare `Koine WASM export "<m>" is not a function` becomes
 * the actionable stale-bundle "rebuild" message; any other error is returned unchanged. Shared by the
 * worker proxy and by direct `client.call(...)` callers (e.g. the transport's cancellable diagnose,
 * #353) so both surface the same guidance instead of the cryptic raw error.
 */
export function mapWorkerCallError(err: unknown): Error {
