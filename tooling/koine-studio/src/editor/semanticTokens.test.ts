...
// with a CSS class. Runs on a CodeMirror `Text` (no EditorView/DOM), so the delta arithmetic (same-line
// vs new-line column reset), the legend index → class map, the declaration modifier bit, and the
// defensive drops (bad index, zero length, out-of-doc) are all pinned here. This is the unit that the
// editor's semanticTokensExtension wraps with a live view.
