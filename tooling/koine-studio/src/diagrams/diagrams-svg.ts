 ...
 * on the diagrams container and seeds the matching starter into the active buffer (non-destructive: it
 * replaces an untouched BLANK seed, otherwise appends), which re-renders the canvas with real nodes.
 */
...
/**
 * Build the empty-canvas state: the inviting "blank vellum" the visual editor shows before the model has
 * anything to draw. Real DOM (not innerHTML) so the concept tiles carry click + keyboard handlers that
 * dispatch {@link EMPTY_STATE_PICK_EVENT}; ide.ts seeds the picked starter and the canvas re-renders.
 */
