...
import { fileUriToPath, pathToFileUri } from "@/shell/ideUtils";
...
export function init(hooks: IdeHooks = {}): () => void {
    ...
    const initialDoc =
        (shared?.kind === "single" ? shared.text : null) ??
        legacyScratch ??
        SEED;
    ...
    render(
        <WorkspaceProblemsBadge store={appStore} />,
        domById("sb-problems-host"),
    );
        ...
        if (!storeInspectorHost) {
            ...
            try {
                const { StoreInspector } =
                    await import("@/shell/StoreInspector");
    ...
    const reviewStore = createReviewStore(
        platform,
        () => workspace.folderRootToken() || null,
    );
    ...
    const editorSession = createEditorSession({
        ...
        onDocChange: (change, doc, file) =>
            reviewStore.remap(file, change, doc),
    });
    ...
    const unsubReviewStore = reviewStore.subscribe(() =>
    ...
    const explorer = createExplorer({
        ...
        onNewFile: (parentDirToken, name) =>
            void workspace.handleNewFile(parentDirToken, name),
        onNewFolder: (parentDirToken, name) =>
            void workspace.handleNewFolder(parentDirToken, name),
        onRename: (entry, newName) =>
            void workspace.handleRename(entry, newName),
        onDelete: (entry) => void workspace.handleDelete(entry),
        onDuplicate: (entry) => void workspace.handleDuplicate(entry),
        onMove: (entry, destDirToken) =>
            void workspace.handleMove(entry, destDirToken),
        isActive: (token) => pathToFileUri(token) === workspace.activeUri(),
        isDirty: (token) =>
            workspace.buffers.get(pathToFileUri(token))?.dirty ?? false,
    ...
    function navigateToDefinition(loc: Location): void {
        if (
            loc.uri &&
            loc.uri !== workspace.activeUri() &&
            workspace.buffers.has(loc.uri)
        ) {
    ...
    const controller = createInspectorController({
        ...
        loadActiveContext,
        setStatus,
        onRenameElement: (element, newName) =>
            void renameElement(element, newName),
        onSaveElementDescription: (element, text) =>
            void saveInspectorDescription(element, text),
        onSaveGlossaryDescription: (entry, text) =>
            saveDescription(entry, text),
        onApplyStructuredEdit: (edit, successMsg) =>
            void canvasWrite.applyStructuredEdit(edit, successMsg),
        onAddConstruct: (kind) =>
            void canvasWrite.applyDiagramAddType({ kind }),
        onAddAnnotation: (kind) => canvasWrite.createCanvasAnnotation(kind),
        onAddAggregateMember: (kind, aggregateQn) =>
            void canvasWrite.applyDiagramAddAggregateMember(kind, aggregateQn),
        onExportDiagram: (format) =>
            void exportShare.exportActiveDiagram(format),
    ...
    const selection = {
        set: (element: SelectedElement | null) =>
            appStore.getState().setSelection(element),
    };
    ...
    async function saveDescription(
        ...
        if (
            result.uri &&
            result.uri !== workspace.activeUri() &&
            workspace.buffers.has(result.uri)
        )
            workspace.activateFile(result.uri);
    ...
    async function saveInspectorDescription(
        element: InspectorElement,
        text: string,
    ): Promise<void> {
        try {
    ...
    domById<HTMLButtonElement>("btn-check").addEventListener(
        "click",
        () => void controller.runCheck(),
    );
    ...
    if (!platform.canOpenFolders) {
        ...
        openFolderBtn.title =
            "Opening a folder needs a Chromium-based browser (try Chrome or Edge)";
    }
    ...
    workspace = createWorkspaceController({
        ...
        getFormatOnSave: () =>
            effectiveSettings(settings, wsKey()).formatOnSave,
        ...
        onFolderOpened: () => {
            ...
            void reviewStore
                .load()
                .then(() => editorSession.refreshReviewDecorations());
        },
    ...
    window.addEventListener("beforeunload", (e) =>
        handleBeforeUnload(e, () => workspace.anyDirty()),
    );
    ...
    async function newModel(): Promise<void> {
        ...
        try {
            for (const entry of await platform.listEntries(token))
    ...
    function searchLabelForUri(uri: string): string {
        ...
        if (
            root &&
            (token.startsWith(root + "/") || token.startsWith(root + "\\"))
        ) {
    ...
    async function revealSearchMatch(uri: string, match: Match): Promise<void> {
        ...
        const startOffset = Math.min(
            doc.line(startLine).from + match.column,
            doc.length,
        );
        ...
        editor.gotoRange(
            {
                line: startInfo.number - 1,
                character: startOffset - startInfo.from,
            },
    ...
    const search = createSearchPanel({
        ...
        getActiveBuffers: () => {
            ...
            for (const buf of workspace.buffers.values())
                m.set(buf.uri, buf.text);
            return m;
        },
    ...
    const panelHost = createPanelHost({
        prefsCallbacks,
        settingsCategory: () =>
            appStore.getState().settingsCategory ?? undefined,
    ...
    const exportShare = createExportShare({
        ...
        refreshStatusFromDiagnostics: () =>
            editorSession.updateStatus(
                editorSession.diagnosticsFor(workspace.activeUri()),
            ),
    ...
    const commandWiring = createCommandWiring({
        ...
        workspace: {
            saveAllDirty: () => void workspace.saveAllDirty(),
            buffers: workspace.buffers,
        },
        ...
        exportActiveDiagram: (format) =>
            void exportShare.exportActiveDiagram(format),
        copyActiveDiagramMermaid: () =>
            void exportShare.copyActiveDiagramMermaid(),
    ...
    const lifecycleBoot = createLifecycleBoot({
        ...
        legacyScratch,
        seed: SEED,
        importSharedWorkspace: (files, active) =>
            exportShare.importSharedWorkspace(files, active),
        openWorkspaceWith1File: (text) =>
            workspace.openWorkspaceWith1File(text),
        openFolderPath: (folder, opts) =>
            workspace.openFolderPath(folder, opts),
        isAutoRestorableToken: (token) => platform.isAutoRestorableToken(token),
        openHostDefaultWorkspaceFlow: (seed) =>
            workspace.openDefaultWorkspaceFlow(seed),
        setStatus,
