...
import { parseNewFileKey } from '@/ai/editSession';
...
export function createWorkspaceBuffers(ctx: WorkspaceModuleCtx) {
  ...
  // multi-root workspace hold the same relPath — or a `new:<relPath>`/`new-in:<root>:<relPath>` key
  // (#1132) for a file that doesn't exist yet. An EXISTING buffer is updated in place — reflected in
  // the editor when it's the active one (so the change shows + fires onChange), synced to the LSP,
  // persisted, and left clean UNLESS a keystroke landed mid-write (mirrors saveActive/saveAllDirty). A
  // plain `new:` key is CREATED under the PRIMARY root (the legacy behavior, for a caller with no root
  // to name); a `new-in:` key names its OWN destination root, validated against the live root list —
  // an honest null (never a silent primary-root fallback) when that root is no longer part of the
  // workspace. Any other key — including a bare relPath — is unknown: return null rather than guess
  // across roots or silently create. Returns the file uri, or null on failure.
  async function applyFileEdit(key: string, body: string): Promise<string | null> {
    ...
    const parsed = parseNewFileKey(key);
    if (!parsed) return null; // unknown key: not an open buffer, not a new-file key
    const { relPath, root } = parsed;
    // A plain `new:` key names no root of its own — the legacy primary-root behavior. A `new-in:` key
    // names its OWN root (#1132): validate it against the LIVE root list rather than trust a root that
    // may have been removed since the model proposed it — an honest null, never a silent fallback to
    // the primary root the model/user never chose.
    const owningRoot = root === null ? st().roots[0] : st().roots.includes(root) ? root : undefined;
    if (!owningRoot) return null;
    try {
      const token = await platform.createFile(owningRoot, relPath, body); // new file under the chosen root
