// DeckCard — one surface card on the deck stage: a header (icon + label + hoisted facet sub-strip +
// tag + close) over a body slot. The body is host-agnostic: a real center-host `HTMLElement`
// (re-parented in via a ref) in the app, or mock `children` in Storybook.
//
// IMPORTANT: the card ROOT's dynamic layout classes (ghost / in-pair / is-selected / is-primary /
// is-secondary) and its inline geometry are owned by the imperative FLIP engine (deckLayout.ts), NOT
// by Preact — this component never sets them on the root, so reconciliation can't fight the animation.
// The header's selected/pair styling cascades from those root classes via CSS. `inPair` / `isSelected`
// here drive CONTENT only (whether the close button is reachable, aria state).
import type { ComponentChildren, JSX } from 'preact';
import { IconClose, type DeckSurface } from '@/shell/deck/surfaces';

export interface DeckCardProps {
  surface: DeckSurface;
  /** The surface's active facet value (TechView/OutputTab/DocsView as a string), or null when none. */
  ...
  onClose(): void;
  /** Collect the root element for the FLIP engine. */
  rootRef(el: HTMLElement | null): void;
  /** The real surface body to host (re-parented into the card). Omit in Storybook and pass `children`. */
  hostEl?: HTMLElement;
