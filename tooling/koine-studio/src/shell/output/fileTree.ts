...
/** One node of a generated-output file tree: a folder (with nested children) or a leaf file (with its emitted contents). */
...
interface FolderBuilder {
  ...
  files: Map<string, { path: string; contents: string }>;
}
...
function toNodes(folder: FolderBuilder): TreeNode[] {
  ...
  const fileNodes: TreeNode[] = [...folder.files.entries()]
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([name, file]) => ({ kind: 'file', name, path: file.path, contents: file.contents }));
...
export function buildFileTree(files: EmitFile[]): TreeNode[] {
  ...
  for (const file of files) {
    ...
    current.files.set(fileName, { path: file.path, contents: file.contents });
  }
