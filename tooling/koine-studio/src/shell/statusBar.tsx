...
export function createStatusBar(deps: StatusBarDeps): StatusBarHandle {
  ...
  // Disposed by dispose(); consulted after every `await` in refreshBranch so an in-flight gitStatus that
  // resolves post-teardown cannot write into a torn-down bar (the existing token guard covers a folder
  // switch, not a teardown — the token can be unchanged). The shared #1352 primitive, disposed-only shape
  // (no createSequence() — this file has no per-request staleness counter to replace).
  ...
  async function refreshBranch(): Promise<void> {
    ...
    try {
      ...
      if (lifecycle.isDisposed()) return; // the bar was torn down while this fetch was in flight — do not write.
      ...
      if (lifecycle.isDisposed()) return; // likewise on the failure path — a torn-down bar's segment must stay put.
