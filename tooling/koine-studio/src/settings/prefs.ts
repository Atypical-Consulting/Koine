...
export function stringListInput(
    ...
    // Chip area: one <li> per token, each with a labelled remove button.
    const chipArea = document.createElement("ul");
    chipArea.className = "koi-string-list-chips";
    chipArea.setAttribute("role", "list");
    ...
    const addInputId = `koi-sl-${ariaLabel
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/(^-|-$)/g, "")}`;
...
export function mountPreferencesPane(
    ...
    >(
        ...
        function applyEnabled(): void {
            ...
            for (const b of scopeSeg.el.querySelectorAll<HTMLButtonElement>(
                ".koi-seg",
            ))
                b.disabled = !enabled;
        }
    ...
    function accentPicker(onSelect: (value: AccentName) => void): {
        el: HTMLElement;
    ...
    function langPicker(onSelect: (value: PreviewTarget) => void): {
        el: HTMLElement;
