// Workspace/session state (issue #988): the recent-folders list, the legacy single-file scratch
// migration helpers, the workspace center-pane and Deck layout, the last-opened-workspace pointer, the
// last-session resume snapshot, the per-workspace active-context scope, and the per-workspace assistant
// transcript. Split out of persistence.ts, which now re-exports this module's public surface via
// `export * from './workspaceState'` so `@/settings/persistence` stays the unchanged import path for
...
//
// Note (#984): transcript-slice hydration is built ON TOP of this trio (loadChat/saveChat/clearChat) —
// extend the storage/coercion logic here rather than duplicating a parallel read/write path elsewhere.
