...
describe("stringListInput — reusable string-list control (#785)", () => {
    ...
    const removeButtons = () => [
        ...container.querySelectorAll<HTMLButtonElement>(
            ".koi-string-list-remove",
        ),
    ];
    // Returns the text input for adding a new token.
    const addInput = () =>
        container.querySelector<HTMLInputElement>(".koi-string-list-input")!;
    // Returns the add button.
    const addButton = () =>
        container.querySelector<HTMLButtonElement>(".koi-string-list-add")!;
    // Returns all chip label spans.
    const chipLabels = () =>
        [
            ...container.querySelectorAll<HTMLElement>(
                ".koi-string-list-chip-label",
            ),
        ].map((s) => s.textContent);
...
describe("Settings → Advanced: terminal shell args row (#785)", () => {
    ...
    const advancedPanel = () =>
        document.querySelector<HTMLElement>("#koi-settings-panel-advanced")!;
    const shellArgsRow = () =>
        advancedPanel()?.querySelector<HTMLElement>(".koi-string-list-control");
    const chips = () =>
        [
            ...(advancedPanel()?.querySelectorAll<HTMLElement>(
                ".koi-string-list-chip-label",
            ) ?? []),
        ].map((s) => s.textContent);
    const addInput = () =>
        advancedPanel()?.querySelector<HTMLInputElement>(
            ".koi-string-list-input",
        );
    const addButton = () =>
        advancedPanel()?.querySelector<HTMLButtonElement>(
            ".koi-string-list-add",
        );
    const removeButtons = () => [
        ...(advancedPanel()?.querySelectorAll<HTMLButtonElement>(
            ".koi-string-list-remove",
        ) ?? []),
    ];
    ...
    it("hides (or omits) the Terminal shell args row when hasIntegratedTerminal is false", () => {
        ...
        expect(
            row === null ||
                (row as HTMLElement).closest("[hidden]") !== null ||
                row.hidden,
        ).toBe(true);
    });
