...
const mcpClientSelect = () =>
    document.querySelector<HTMLSelectElement>(
        "#koi-settings-panel-mcp .koi-select",
    )!;
const snippet = () =>
    document.querySelector<HTMLDivElement>(".koi-mcp-snippet")!;
const status = () => document.querySelector<HTMLElement>(".koi-mcp-status")!;
const endpointUrl = () =>
    document.querySelector<HTMLInputElement>(".koi-mcp-control .koi-text")!;
const testBtn = () =>
    [
        ...document.querySelectorAll<HTMLButtonElement>(
            "#koi-settings-panel-mcp .koi-set-action",
        ),
...
describe("Settings → MCP panel", () => {
    ...
    it("renders the recipe as a highlighted CodeMirror view and Copy writes the exact snippet", async () => {
        ...
        expect(
            document.querySelector(".koi-mcp-snippet .cm-editor"),
        ).not.toBeNull();

        const expected = MCP_CLIENTS.find((c) => c.id === "lm-studio")!.snippet(
            MCP_URL_PLACEHOLDER,
        );
        const copyBtn = [
            ...document.querySelectorAll<HTMLButtonElement>(
...
describe("Settings → Output panel", () => {
    ...
    const langOpts = () => [
        ...document.querySelectorAll<HTMLButtonElement>(
            "#koi-settings-panel-output .koi-lang-opt",
        ),
    ];
    ...
    it("renders one option per built-in emit target with C# selected by default", () => {
        ...
        expect(opts.map((b) => b.dataset.value)).toEqual(
            BUILTIN_EMIT_TARGETS.map((t) => t.id),
        );
        const checked = opts.find(
...
describe("Settings → About panel", () => {
    ...
    it("renders About as the last category tab with the wordmark and four links", () => {
        ...
        expect(
            panel.querySelector(".koi-about-wordmark")?.textContent,
        ).toContain("Koine");
        const labels = [...panel.querySelectorAll(".koi-about-link-label")].map(
            (n) => n.textContent,
        );
    ...
    it('refresh("about") lands directly on the About tab', () => {
        ...
        const tab = document.querySelector<HTMLButtonElement>(
            "#koi-settings-tab-about",
        )!;
        expect(tab.getAttribute("aria-selected")).toBe("true");
        expect(
            document.querySelector<HTMLElement>("#koi-settings-panel-about")!
                .hidden,
        ).toBe(false);
    });
    ...
    it("fills the build chip on open", async () => {
        ...
        const chip = document.querySelector<HTMLElement>(
            "#koi-settings-panel-about .koi-about-chip",
        )!;
...
describe("Settings → User/Workspace scope toggle", () => {
    ...
    const scopeGroup = (label: string) =>
        document.querySelector<HTMLElement>(
            `.koi-segmented[aria-label="${label}"]`,
        )!;
    ...
    it("hides/disables every scope control when no workspace is open", () => {
        ...
        for (const label of [
            ...
            for (const b of group.querySelectorAll<HTMLButtonElement>(
                ".koi-seg",
            )) {
    ...
    it("enables the scope control when a workspace is open, defaulting to User", () => {
        ...
        expect(
            scopeBtn(WORD_WRAP_SCOPE, "user").getAttribute("aria-checked"),
        ).toBe("true");
        expect(
    ...
    it("flipping back to User clears the override and restores the user value in the control", () => {
        ...
        expect(loadWorkspaceOverrides(KEY)).toHaveProperty(
            "formatOnSave",
            false,
        );
        ...
        expect(scopeBtn(FOS_SCOPE, "user").getAttribute("aria-checked")).toBe(
            "true",
        );
    });
...
describe("Settings → segmented() keyboard navigation (#732)", () => {
    ...
    const themeGroup = () =>
        document.querySelector<HTMLElement>(
            '.koi-segmented[aria-label="Theme"]',
        )!;
    const themeBtn = (value: "dark" | "light") =>
        themeGroup().querySelector<HTMLButtonElement>(
            `.koi-seg[data-value="${value}"]`,
        )!;
    ...
    it("arrow navigation skips disabled options: a no-workspace scope toggle stays inert", () => {
        ...
        const group = document.querySelector<HTMLElement>(
            '.koi-segmented[aria-label="Word wrap scope"]',
        )!;
        const workspace = group.querySelector<HTMLButtonElement>(
            '.koi-seg[data-value="workspace"]',
        )!;
        const user = group.querySelector<HTMLButtonElement>(
            '.koi-seg[data-value="user"]',
        )!;
        onChange.mockClear();
...
describe("segmented().setDisabled (#791)", () => {
    it("setDisabled(true) sets aria-disabled on the group, adds is-disabled class, disables every button", () => {
        ...
        for (const b of seg.el.querySelectorAll<HTMLButtonElement>(
            ".koi-seg",
        )) {
            expect(b.disabled).toBe(true);
        }
...
describe("keyboard settings", () => {
    ...
    const rows = () => [
        ...document.querySelectorAll<HTMLElement>(
            "#koi-settings-panel-keyboard .koi-kbd-row",
        ),
    ];
    const rowFor = (id: string) =>
        document.querySelector<HTMLElement>(
            `#koi-settings-panel-keyboard .koi-kbd-row[data-binding-id="${id}"]`,
        )!;
    const chordOf = (id: string) =>
        rowFor(id).querySelector<HTMLElement>(".koi-kbd-chord")!;
    const recordBtn = (id: string) =>
        rowFor(id).querySelector<HTMLButtonElement>(".koi-kbd-record")!;
    const resetBtn = (id: string) =>
        rowFor(id).querySelector<HTMLButtonElement>(".koi-kbd-reset")!;
    const conflictOf = (id: string) =>
        rowFor(id).querySelector<HTMLElement>(".koi-kbd-conflict")!;
    const reassignBtn = (id: string) =>
        rowFor(id).querySelector<HTMLButtonElement>(".koi-kbd-reassign")!;
    ...
    it("Reset all clears every override and fires onKeybindingsChanged", () => {
        ...
        document
            .querySelector<HTMLButtonElement>(
                "#koi-settings-panel-keyboard .koi-kbd-reset-all",
            )!
            .click();
    ...
    it('Advanced "Reset to defaults" also clears keybinding overrides', () => {
        ...
        const danger = document.querySelector<HTMLButtonElement>(
            "#koi-settings-panel-advanced .koi-set-danger",
        )!;
        danger.click(); // arm
...
describe("Settings → new namespaced controls (#750): Tab size / Editor font / Temperature", () => {
    ...
    const tabSizeInput = () =>
        document.querySelector<HTMLInputElement>("#koi-set-tab-size")!;
    const editorFontInput = () =>
        document.querySelector<HTMLInputElement>("#koi-set-editor-font")!;
    const temperatureInput = () =>
        document.querySelector<HTMLInputElement>("#koi-set-temperature")!;
...
describe("mountPreferencesPane", () => {
    ...
    it("destroy clears the container and tears down the MCP code view", () => {
        ...
        expect(
            host.querySelector(".koi-mcp-snippet .cm-editor"),
        ).not.toBeNull();
        handle.destroy();
...
// ---------------------------------------------------------------------------
// stringListInput — reusable string-list control (#785)
// ---------------------------------------------------------------------------

describe("stringListInput — reusable string-list control (#785)", () => {
    it("set() renders one chip per value", () => {
        const { el, set } = stringListInput("Shell args", vi.fn());
        set(["-l", "-i"]);
        const chips = el.querySelectorAll(".koi-chip");
        expect(chips).toHaveLength(2);
        expect(chips[0].querySelector("span")!.textContent).toBe("-l");
        expect(chips[1].querySelector("span")!.textContent).toBe("-i");
    });

    it("set([]) renders no chips", () => {
        const { el, set } = stringListInput("Shell args", vi.fn());
        set([]);
        expect(el.querySelectorAll(".koi-chip")).toHaveLength(0);
    });

    it("adding a blank token is a no-op (no commit, no chip)", () => {
        const onCommit = vi.fn();
        const { el, set } = stringListInput("Shell args", onCommit);
        set(["-l"]);
        const input = el.querySelector<HTMLInputElement>(
            ".koi-string-list-input",
        )!;
        input.value = "   ";
        el.querySelector<HTMLButtonElement>(".koi-string-list-add")!.click();
        expect(onCommit).not.toHaveBeenCalled();
        expect(el.querySelectorAll(".koi-chip")).toHaveLength(1);
    });

    it("adding a token commits the full array", () => {
        const onCommit = vi.fn();
        const { el, set } = stringListInput("Shell args", onCommit);
        set(["-l", "-i"]);
        const input = el.querySelector<HTMLInputElement>(
        ...
        expect(el.querySelectorAll(".koi-chip")).toHaveLength(3);
    });

    it("token value is trimmed before commit", () => {
        const onCommit = vi.fn();
        const { el } = stringListInput("Shell args", onCommit);
        const input = el.querySelector<HTMLInputElement>(
            ".koi-string-list-input",
        )!;
        input.value = "  -l  ";
    ...
    it("removing a chip commits the remaining array", () => {
        ...
        const removeBtns =
            el.querySelectorAll<HTMLButtonElement>(".koi-chip button");
        removeBtns[1].click();
        expect(onCommit).toHaveBeenCalledWith(["-l", "-x"]);
        expect(el.querySelectorAll(".koi-chip")).toHaveLength(2);
    });

    it("chip remove buttons have accessible aria-label", () => {
        const { el, set } = stringListInput("Shell args", vi.fn());
        set(["-l"]);
        const removeBtn =
            el.querySelector<HTMLButtonElement>(".koi-chip button")!;
        expect(removeBtn.getAttribute("aria-label")).toBe("Remove -l");
    });

    it("add input carries the aria-label", () => {
        const { el } = stringListInput("My label", vi.fn());
        const input = el.querySelector<HTMLInputElement>(
            ".koi-string-list-input",
        )!;
    ...
    it("pressing Enter in the add input commits the token", () => {
        const onCommit = vi.fn();
        const { el } = stringListInput("Shell args", onCommit);
        const input = el.querySelector<HTMLInputElement>(
            ".koi-string-list-input",
...
// ---------------------------------------------------------------------------
// Settings → Advanced: terminal shell args row (#785)
// ---------------------------------------------------------------------------
...
describe("Settings → Advanced: terminal shell args row (#785)", () => {
    ...
    function openPrefsWithTerminal(
        ...
        const onChange = vi.fn();
        const pane = mountPreferencesPane(container, {
            onChange,
    ...
    it("shell args row is hidden when hasIntegratedTerminal is omitted", () => {
        ...
        pane.refresh();
        const row = container
            .querySelector<HTMLElement>(".koi-string-list")
    ...
    it("adding a token commits via patchSettings and fires onChange", () => {
        saveSettings({ ...DEFAULT_SETTINGS, terminalShellArgs: [] });
        const onChange = vi.fn();
        openPrefsWithTerminal({ onChange }, true);
        ...
        expect(loadSettings().terminalShellArgs).toContain("-x");
        expect(onChange).toHaveBeenCalled();
    });

    it("removing a chip commits via patchSettings and fires onChange", () => {
        saveSettings({ ...DEFAULT_SETTINGS, terminalShellArgs: ["-l", "-i"] });
        const onChange = vi.fn();
        openPrefsWithTerminal({ onChange }, true);

        const removeBtns =
            container.querySelectorAll<HTMLButtonElement>(".koi-chip button");
        removeBtns[0].click();

        const saved = loadSettings().terminalShellArgs;
        expect(saved).not.toContain("-l");
        expect(onChange).toHaveBeenCalled();
    });

    it("populate() re-syncs the chip list on refresh", () => {
        saveSettings({ ...DEFAULT_SETTINGS, terminalShellArgs: ["-l"] });
        const pane = openPrefsWithTerminal({}, true);
    ...
    it("Reset clears terminalShellArgs to []", () => {
        saveSettings({ ...DEFAULT_SETTINGS, terminalShellArgs: ["-l", "-i"] });
        openPrefsWithTerminal({}, true);

        // Arm the reset button (first click arms, second resets)
