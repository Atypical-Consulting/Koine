...
describe('stringListInput — reusable string-list control (#785)', () => {
  ...
  it('adding a blank token is a no-op (no commit, no chip)', () => {
    ...
    set(['-l']);
    const input = el.querySelector<HTMLInputElement>('.koi-string-list-input')!;
    input.value = '   ';
  ...
  it('removing a chip commits the remaining array', () => {
    ...
    const removeBtns = el.querySelectorAll<HTMLButtonElement>('.koi-chip button');
    removeBtns[1].click();
  ...
  it('chip remove buttons have accessible aria-label', () => {
    const { el, set } = stringListInput('Shell args', vi.fn());
    set(['-l']);
    const removeBtn = el.querySelector<HTMLButtonElement>('.koi-chip button')!;
...
describe('Settings → Advanced: terminal shell args row (#785)', () => {
  ...
  it('adding a token commits via patchSettings and fires onChange', () => {
    ...
    const input = container.querySelector<HTMLInputElement>('.koi-string-list-input')!;
    const addBtn = container.querySelector<HTMLButtonElement>('.koi-string-list-add')!;
    input.value = '-x';
    addBtn.click();
  ...
  it('removing a chip commits via patchSettings and fires onChange', () => {
    ...
    const removeBtns = container.querySelectorAll<HTMLButtonElement>('.koi-chip button');
    removeBtns[0].click();
