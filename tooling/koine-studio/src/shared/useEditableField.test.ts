...
describe('useEditableField', () => {
  ...
  test('Enter blurs the element; the commit happens exactly once, via the blur handler', async () => {
    const onCommit = vi.fn();
    const { container } = render(h(Field, { identity: 'id', value: 'Order', onCommit }));
    const input = getInput(container);

    await type(input, 'PurchaseOrder');
    await act(() => {
  ...
  test('Escape resets the DOM value to the committed value, blurs, and never commits', async () => {
    const onCommit = vi.fn();
    const { container } = render(h(Field, { identity: 'id', value: 'Order', onCommit }));
    const input = getInput(container);
  ...
  test('blur with an unchanged value does not commit', async () => {
    const onCommit = vi.fn();
    const { container } = render(h(Field, { identity: 'id', value: 'Order', onCommit }));
    const input = getInput(container);

    await type(input, 'Order'); // unchanged
  ...
  test('blur with a blank value does not commit and resets the DOM to the committed value', async () => {
    const onCommit = vi.fn();
    const { container } = render(h(Field, { identity: 'id', value: 'Order', onCommit }));
    const input = getInput(container);
  ...
  test('blur with a changed, non-blank value commits the trimmed text exactly once', async () => {
    const onCommit = vi.fn();
    const { container } = render(h(Field, { identity: 'id', value: 'Order', onCommit }));
    const input = getInput(container);
  ...
  test('commitBlank: true lets a cleared field commit the empty string (the Description-clearing case)', async () => {
    const onCommit = vi.fn();
  ...
  test('commitBlank: true still no-ops when the blank value is unchanged', async () => {
    const onCommit = vi.fn();
  ...
  test('an identity change resets the field to the new value even without a remount (no key spread)', async () => {
    const onCommit = vi.fn();
    ...
    await type(input, 'RenamedSalesItem');
  ...
  test('a re-render with the SAME identity never clobbers in-progress typing', async () => {
    const onCommit = vi.fn();
    const { container, rerender } = render(
  ...
  test('with the key spread (the normal caller shape), an identity change remounts with the fresh value', async () => {
    const onCommit = vi.fn();
...
describe('useEditableField — transient-session mode (onCancel present)', () => {
  test('Enter with an invalid value keeps the edit open, fires onInvalid, and neither blurs nor commits', async () => {
    const onCommit = vi.fn();
  ...
  test('Enter with a valid, changed value blurs and commits exactly once (via the blur handler)', async () => {
    const onCommit = vi.fn();
    const onCancel = vi.fn();
    const { container } = render(
  ...
  test('Escape cancels the session (onCancel), never commits', async () => {
    const onCommit = vi.fn();
    const onCancel = vi.fn();
    const { container } = render(
  ...
  test('blur on a blank value cancels the session (onCancel), never commits', async () => {
    const onCommit = vi.fn();
    const onCancel = vi.fn();
    const { container } = render(
      h(SessionField, { identity: 'create:ROOT:file', value: '', onCommit, onCancel, validate: noSlash }),
  ...
  test('blur on an invalid value cancels the session (onCancel), never commits (no trap)', async () => {
    const onCommit = vi.fn();
    const onCancel = vi.fn();
    const onInvalid = vi.fn();
    const { container } = render(
  ...
  test('blur on a valid, changed value commits the trimmed text exactly once, never cancels', async () => {
    const onCommit = vi.fn();
    const onCancel = vi.fn();
    const { container } = render(
      h(SessionField, { identity: 'create:ROOT:file', value: '', onCommit, onCancel, validate: noSlash }),
  ...
  test('blur on a valid but UNCHANGED value cancels the session (no-op rename closes, skips commit)', async () => {
    const onCommit = vi.fn();
    const onCancel = vi.fn();
    const { container } = render(
      h(SessionField, { identity: 'rename:X', value: 'Order', onCommit, onCancel, validate: noSlash }),
  ...
  test('blur on a DISCONNECTED element cancels the session and never commits (upstream removal mid-edit)', async () => {
    const onCommit = vi.fn();
    const onCancel = vi.fn();
    const { container } = render(
      h(SessionField, { identity: 'rename:X', value: 'Order', onCommit, onCancel, validate: noSlash }),
    ...
    await type(input, 'RenamedButGone'); // a valid, changed value — would otherwise commit
  ...
  describe('persistent mode (no onCancel) is unchanged even when validate/onInvalid are passed', () => {
    ...
    test('Enter commits via blur and ignores validate/onInvalid (persistent field, not a session)', async () => {
      const onCommit = vi.fn();
      const onInvalid = vi.fn();
      const { container } = render(
      ...
      await type(input, 'bad/name'); // would be rejected in session mode
      await act(() => {
        fireEvent.keyDown(input, { key: 'Enter' });
      });
    ...
    test('Escape resets the DOM value and blurs (persistent behavior), never cancels/commits', async () => {
      const onCommit = vi.fn();
      const { container } = render(
