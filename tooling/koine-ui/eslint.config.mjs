...
export default tseslint.config(
  {
    ...
    rules: {
      ...
      'no-restricted-syntax': ['error', {
        // Any `x.innerHTML = …` (and `+=`). The one sanctioned sink is el()'s documented `html:` escape
        // hatch (already-trusted/escaped markup), annotated inline; everything else must use textContent.
        selector: "AssignmentExpression[left.property.name='innerHTML']",
        message: 'Assigning innerHTML is an XSS sink. Use textContent/el(); only already-trusted/escaped markup, behind a justified disable.',
      }],
    },
  },
