/**
 * Unit tests for `classifyBootOutcome` (scripts/smoke-boot.mjs) — issue #492.
 *
 * The full smoke-test drives a real Chromium against a built `dist/`, which is too heavy for the unit
 * suite. The *verdict* logic is split out as a pure function so its branches — especially the
 ...
 * run-directly check), so this import never starts Playwright. Mirrors
 * tooling/koine-studio/scripts/smoke-boot.test.mjs.
 *
...
describe('classifyBootOutcome (issue #492)', () => {
  ...
  test('worker `error` event WITH assets fetched → still load/parse failure (worker-error predicate isolated)', () => {
    ...
    // load/parse — never fall through to "boot hung".
  ...
  test('timeout with assets fetched but never settled → boot hung (the #492 symptom)', () => {
