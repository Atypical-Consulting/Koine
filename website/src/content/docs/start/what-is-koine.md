...
The model is written once and stays free of any host-language concepts. **This release emits C#**, with **TypeScript**, **Python 3.11+**, **PHP 8.1**, and **Rust** emitters also shipping today (all Phase 1: tactical core) — the parser and semantic model are kept strictly target-agnostic so a new backend is a new emitter, not a rewrite.
