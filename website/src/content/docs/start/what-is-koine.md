...
The model is written once and stays free of any host-language concepts. **This release emits C#**, with **TypeScript**, **Python 3.11+**, **PHP 8.1**, and **Java 17** (each now covering the full tactical core *and* the strategic/CQRS layer — read models, queries, policies, state machines, context maps/ACL), plus **Rust** and **Kotlin 2.x/JVM** emitters also shipping today — the parser and semantic model are kept strictly target-agnostic so a new backend is a new emitter, not a rewrite.
