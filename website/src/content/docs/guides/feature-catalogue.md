...
| Command returning a value | `command cancel(): OrderId { …; result id }` | a typed method (`public <T> Name(…)`) that returns the `result` expression as its terminal statement (the create-and-return-id idiom). When that expression is *also* a whole `emit`/`publish` payload argument, **every** code target binds it to one `__result` local, so it is [evaluated exactly once](/Koine/reference/commands-events-state/#1133-returning-a-value) and the recorded payload and the returned value are the same value | — |
