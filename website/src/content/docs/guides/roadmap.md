...
| **Java** | Full tactical **and** strategic/CQRS layer — value objects & events as validating `record`s, smart enums, entities/aggregates with invariant-guarded behaviors and state-machine transition guards, generated IDs, a sealed `DomainEvent`, repository interfaces, read models with static projections, queries + `QueryHandler` seams, service boundaries, policies, context-map/ACL translators, integration-subscriber seams, and an opt-in infrastructure layer at parity with TypeScript/Python (in-memory repositories, a unit of work closing the outbox producer loop, transactional outbox, validation/transaction pipeline behaviors, and a per-context composition helper); multi-context references package-qualify; dependency-free Java 17 | `javac --release 17` |
