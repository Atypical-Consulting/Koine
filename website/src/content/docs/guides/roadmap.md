...
| **Java** | Full tactical **and** strategic/CQRS layer — value objects & events as validating `record`s, smart enums, entities/aggregates with invariant-guarded behaviors and state-machine transition guards, generated IDs, a sealed `DomainEvent`, repository interfaces, read models with static projections, queries + `QueryHandler` seams, service boundaries, policies, context-map/ACL translators, integration-subscriber seams, and an opt-in infrastructure layer (in-memory repositories + transactional outbox); multi-context references package-qualify; dependency-free Java 17 | `javac --release 17` |
