...
| `--layers <list>` | `domain` | Comma-separated output layers (`domain`, `application`, `api`). `application` implies `domain`; `api` (ASP.NET Minimal API endpoints binding each command/query to its handler) implies `application`. C# only. See [the Application layer](#the-c-application-layer). |
...
- **Paths.** An entity **command** becomes a `POST` whose JSON request body is built from its parameters; a **`create` factory** becomes a `POST` the same way, with the created aggregate as its `200`; a **query** becomes a `GET` whose criteria become query `parameters` and whose `200` response references the result schema. Operation paths are kebab-cased. Any of the three annotated with [`@route`/a verb/`@auth`](/Koine/reference/application-cqrs/#159-api-annotations) overrides its path and verb key and gains a per-operation `security` requirement — the same derivation the C# `api` layer uses, so the spec and the endpoints never disagree.
