...
**Keys read today.** Only two flat keys are honoured: `target` (`csharp`, `typescript`, `python`, `php`, `rust`, `glossary`, or `docs`) and `out` (the output directory). Every other key is **silently ignored**, which keeps the file forward-compatible — older tooling tolerates a newer config.
