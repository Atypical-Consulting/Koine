...
- The annotations attach to `command` and `query` only. A [factory (§12)](/Koine/reference/factories/) keeps the conventional `POST /{entity}/{factory}` with no override axis — but it is not invisible to the rest of this section: it still gets its own `openapi` operation (a `200` for the created aggregate, a `400` for a precondition/invariant violation) and still claims its route in the `KOI1211` check below. A `usecase` has no HTTP surface at all.
