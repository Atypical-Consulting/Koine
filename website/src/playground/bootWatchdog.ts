// Playground worker boot watchdog (issues #510 / #357 / #358). Extracted from koine.worker.ts so it
// can be unit-tested without triggering the worker module's top-level boot side effects.
//
...
// watchdog converts that silent infinite hang into an explicit `boot-failure` so the host fails fast
// (and falls back) and the failure is named.
//
// This is an intentional, plan-sanctioned copy of Studio's bootWatchdog.ts — the website/ and
// tooling/koine-studio/ are separate Vite build roots, so cross-package imports would resolve against
// the wrong root (#492 brainstorm). Mirrors tooling/koine-studio/src/host/browser/bootWatchdog.ts.
 ...
 *  - OR a watchdog `boot-failure` if it does neither within `timeoutMs` (the silent hang).
 *
