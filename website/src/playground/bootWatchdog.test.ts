...
describe('broadcastBootSignal (playground worker watchdog · #510 / #357)', () => {
  ...
  it('runs onReady BEFORE emitting `ready` — the message loop must be wired before the host is told it is up', async () => {
    // The whole #357/#492 fix hinges on this ordering: the worker installs its `message` listener
    // (onReady) before posting `ready`, so no RPC the host sends on `ready` can arrive before a
    // listener exists.
    const order: string[] = [];
  ...
  it('fires a watchdog `boot-failure` when boot never settles (the hang) and never signals ready', () => {
    const signals: WorkerSignal[] = [];
    const onReady = vi.fn();
    const ft = fakeTimers();
