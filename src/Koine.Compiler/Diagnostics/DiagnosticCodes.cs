...
public static class DiagnosticCodes
{
    ...
    public static readonly IReadOnlyDictionary<string, DiagnosticDescriptor> Catalogue =
        new Dictionary<string, DiagnosticDescriptor>
        {
            ...
            [NarrowingConversionInDerivedMember] = D(NarrowingConversionInDerivedMember, "A derived member's initializer infers to a wider numeric type (Decimal) than its narrower declared type (Int); no target implicitly narrows Decimal to Int (C#'s CS0266 analogue), so the conversion must be made explicit.", DiagnosticCategory.Expressions, DiagnosticSeverity.Error),
