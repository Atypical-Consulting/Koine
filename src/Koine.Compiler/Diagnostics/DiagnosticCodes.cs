...
public static class DiagnosticCodes
{
    ...
    public const string DerivedMemberCycle = "KOI0110";
    ...
    public const string DuplicateSpecPredicate = "KOI1007";
    ...
    public static readonly IReadOnlyDictionary<string, DiagnosticDescriptor> Catalogue =
        new Dictionary<string, DiagnosticDescriptor>
        {
            ...
            [DerivedMemberCycle] = D(DerivedMemberCycle, "Derived members form a reference cycle; the generated accessors would recurse forever.", DiagnosticCategory.Declarations, DiagnosticSeverity.Error),
            ...
            [DuplicateSpecPredicate] = D(DuplicateSpecPredicate, "Two specs in the same context have names that normalize to the same emitted predicate.", DiagnosticCategory.Specs, DiagnosticSeverity.Error),
            ...
            [InvalidRouteOverride] = D(InvalidRouteOverride, "A command's, query's or factory's '@route' names no path, or a malformed one: not absolute (it must start with '/'), containing whitespace or control characters, or with unbalanced, nested, or empty '{}' route parameters.", DiagnosticCategory.Cqrs, DiagnosticSeverity.Error),
            [MultipleVerbAnnotations] = D(MultipleVerbAnnotations, "A command, query or factory carries more than one HTTP verb annotation ('@get'/'@post'/'@put'/'@delete'/'@patch').", DiagnosticCategory.Cqrs, DiagnosticSeverity.Error),
            [EmptyAuthRole] = D(EmptyAuthRole, "A command's, query's or factory's '@auth' names no role, or a blank one.", DiagnosticCategory.Cqrs, DiagnosticSeverity.Error),
            [DuplicateApiRoute] = D(DuplicateApiRoute, "Two declarations (command, query or factory) in one context resolve to the same HTTP route AND verb; the openapi document would carry a duplicate mapping key and the api layer would map two ambiguous endpoints.", DiagnosticCategory.Cqrs, DiagnosticSeverity.Error),
            [DuplicateApiAnnotation] = D(DuplicateApiAnnotation, "A command, query or factory repeats a single-valued API annotation ('@route' or '@auth'); only the last would take effect.", DiagnosticCategory.Cqrs, DiagnosticSeverity.Error),
            ...
            [VersionAnnotationOnCommand] = D(VersionAnnotationOnCommand, "A command or factory carries a '@since'/'@deprecated' evolution annotation; those apply to type declarations, and neither is one, so it would be discarded.", DiagnosticCategory.Cqrs, DiagnosticSeverity.Error),
            [UnboundRouteToken] = D(UnboundRouteToken, "A '@route' template names a '{token}' that binds to nothing: no parameter/criterion of the declaration is named after it, and (for a command) it is not 'id' either. A query and a factory have no identity fallback at all.", DiagnosticCategory.Cqrs, DiagnosticSeverity.Warning),
